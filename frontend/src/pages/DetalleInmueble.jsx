...
function esFuenteRemaJu(item) {
  return (
    limpiar(item?.fuente) === "remaju" ||
    String(item?.expediente || "").toUpperCase().startsWith("REMAJU-")
  );
}

function puedeVerDetalleRemaJu(item) {
  return Boolean(item?.id && esFuenteRemaJu(item)) || Boolean(item?.urlDetalle);
}
  ...
  <div className="calculadora-resultados">
      ...
      <section className="detalle-grid">
        ...
        <aside className="detalle-panel detalle-recomendacion">
          ...
          {puedeVerDetalleRemaJu(inmueble) ? (
            <a
              href={obtenerUrlDetalleRemaJu(inmueble)}
              target="_blank"
